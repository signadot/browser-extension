...
export const StorageProvider: React.FC<StorageProviderProps> = ({
				...
				if (storedValues[StorageBrowserKeys.signadotUrls] !== undefined) {
					try {
						const urls = JSON.parse(
							storedValues[StorageBrowserKeys.signadotUrls],
						);
						newState.settings = {
							...newState.settings,
							signadotUrls: {
								...
								previewUrl:
									urls.previewUrl || defaultSettings.signadotUrls.previewUrl,
								dashboardUrl:
									urls.dashboardUrl ||
									defaultSettings.signadotUrls.dashboardUrl,
							},
						};
					} catch {
						newState.settings = {
	...
	useEffect(() => {
		...
		if (
			...
			const headersToInject = getHeaders(
				currentRoutingKey,
				headers,
				traceparent,
			);
